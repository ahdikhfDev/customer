    ...
    <div class="bg-slate-50 min-h-screen py-12">
        <div class="container mx-auto px-4">
            ...
            <div id="breadcrumb-container" class="mb-8">
                ...
                <a href="{{ route('layanan') }}?serviceId={{ $serviceId }}"
                    class="text-slate-600 hover:text-[#003d82]">{{ $serviceTitle }}</a>
                <span class="mx-2 text-slate-400">/</span>
                        ...
                        @for ($i = 1; $i <= 4; $i++)
                            ...
                            <div class="flex items-center flex-1">
                                <div class="flex flex-col items-center flex-1">
                                    <div
                                        class="w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 {{ $colorClass }}">
                                        @if ($isCompleted)
                                ...
                                @if ($i < 4)
                                    <div class="h-1 flex-1 mx-2 transition-all duration-300 {{ $isCompleted ? 'bg-green-500' : 'bg-slate-200' }}"
                                        style="margin-bottom: 2.5rem;"></div>
                                @endif
                            </div>
                        @endfor
                    </div>
                ...
                <div class="p-8 max-w-3xl mx-auto mt-12 rounded-lg border bg-card text-card-foreground shadow-sm">
                    ...
                    @if ($userType == 'unknown')
                        <div class="text-center p-8 bg-red-50 border border-red-200 rounded-md">
                            ...
                            <p class="text-slate-600">Terjadi kesalahan. Anda harus memilih kategori pengguna terlebih dahulu.
                            </p>
                            <a href="{{ route('layanan') }}"
                                class="mt-4 inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium h-10 px-4 py-2 bg-[#003d82] text-white hover:bg-[#002754]">
                    ...
                    @else
                        ...
                        <form action="{{ route('application.review') }}" method="POST" enctype="multipart/form-data"
                            class="space-y-6">
                            @csrf
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label for="phone" class="form-label">Nomor Telepon <span
                                                class="required-star">*</span></label>
                                        <input type="tel" id="phone" name="phone" class="form-input" placeholder="08xxxxxxxxxx"
                                            required>
                                    </div>
                                </div>

                                <div>
                                    <label for="question" class="form-label">Keperluan / Pertanyaan <span
                                            class="required-star">*</span></label>
                                    <textarea id="question" name="question" class="form-textarea"
                                        placeholder="Jelaskan keperluan atau pertanyaan Anda secara detail" rows="5"
                                        required></textarea>
                                </div>
                            </div>
                            ...
                            <div class="space-y-4 pt-6 border-t">
                                ...
                                @foreach ($currentData['dokumen'] as $doc)
                                    <div>
                                        ...
                                        <label for="{{ $doc['id'] }}"
                                            class="file-upload-area flex flex-col items-center justify-center">
                                        ...
                                        <input type="file" id="{{ $doc['id'] }}" name="{{ $doc['id'] }}" class="hidden-file-input"
                                ...
                                <div class="flex gap-3">
                                    ...
                                    <div class="text-sm text-slate-700">
                                        ...
                                        <ul class="list-disc list-outside pl-4 space-y-1 text-slate-600">
                                            ...
                                            <li><strong>Jika dokumen tidak sesuai, maka pendaftaran pengunjung akan
                                                    ditolak</strong></li>
                            ...
                            <div class="flex gap-4 pt-6 border-t">
                                <a href="{{ route('layanan') }}?serviceId={{ $serviceId }}"
                                    class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium h-10 px-6 py-2 border border-slate-300 text-slate-700 hover:bg-slate-100 transition-colors gap-2">
                                    <i data-lucide="arrow-left" class="w-4 h-4"></i>
                                    Kembali
                                </a>

                                <button type="submit"
                                    class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium h-10 px-6 py-2 bg-[#003d82] text-white hover:bg-[#002754] transition-colors flex-1 gap-2">
                                    Review & Kirim
